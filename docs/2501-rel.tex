...
\begin{document}
	...
	L'\textcolor{orange}{\underline{amministratore}} si occuperà di aggiungere film alla piattaforma con frequenza settimanale. Sulle recensioni di altri utenti, si può esprimere un parere di utilità che andrà a classificare la recensione in una sorta di "classifica delle recensioni più utili".\\
	...
	\begin{itemize}
		\item "Utente" $\longrightarrow$ Un utilizzatore dell'applicativo che si registra sulla piattaforma ed ha come compito principale la visione e scrittura di recensioni.
	...
	Il testo esplicita come un utente puó scrivere piú di una recensione su vari film, esplicitando il titolo del film, la descrizione della recensione e la sua valutazione. Inoltre, un utente puó classificare, tramite un voto di utilitá, piú di una recensione scritta da un altro utente.
	\begin{figure}[H]
		\centering
		\includegraphics[width=450pt]{ER/cast.png}
		\caption{Schema ER della relazione tra utenza e recensioni.}
	\end{figure}
	\subsection{Schema completo}
	Questo è lo schema finale, è stata realizzata l'entità \textbf{RECENSIONE} per il collegamento tra l'utenza e i film memorizzati, questo perché era necessario dare la possibilità agli utenti di valutare le recensioni già fatte da altri, inoltre, un utente può recensire solamente un film, quindi non è necessario inserire un numero di recensione aggiuntivo.
	\begin{figure}[H]
		\centering
		\includegraphics[width=450pt]{ER/ercompleto.png}
